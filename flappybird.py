...
def getAction(qOld, numTimesVisited):
    """Returns the action the player should take. If it returns 0, it means up
    and if it returns 1, it means down. These numbers correspond to the indices
    of the corresponding Q-values in the qOld parameter.

    Follows the current policy if the state has been visited at least twice
    before, otherwise explores.
    """

    if numTimesVisited <= 1.5:   # explore for first 3 visits of state
        # randomly pick an action
        if flip(0.50):       # go down/do nothing
            action = 1
        else:               # go up
            action = 0
    else:
...
def makeKey(bird, nextPipe):
    """Returns a string that represents a key for that state in the hash table

    State contains horizontal distance between the bird and the pipe divided
    by three, the vertical distance between same two divided by three,
    the bird's slope of motion, and a value corresponding to which half of the
    window the bird is in. See the README.md to hear about my reasoning for
    choosing this state representation.
    """

    distX = int((nextPipe.x - bird.x)/3)

    distY = int((bird.y - (WIN_HEIGHT - nextPipe.bottom_height_px))/3)

    if bird.msec_to_climb > 0:
        frac_climb_done = 1 - bird.msec_to_climb/Bird.CLIMB_DURATION
        nextY = bird.y - (Bird.CLIMB_SPEED * frames_to_msec(1) *
                   (1 - math.cos(frac_climb_done * math.pi)))
        msec_to_climb_left = bird.msec_to_climb - frames_to_msec(1)
    else:
        nextY = bird.y + Bird.SINK_SPEED * frames_to_msec(1)

    nextX = bird.x + ANIMATION_SPEED * frames_to_msec(1)
    birdSlope = -1 * round((nextY - bird.y)/(nextX - bird.x), 1)

    # WIN_HEIGHT = 512 so 256 divides it into 2
    hashKey = str(distY) + " " + str(birdSlope) + " " + str(distX) + " " + str(int(bird.y/HALF_HEIGHT))

    return hashKey

def getQold(ht, hashKey):
    if hashKey in ht:
        [actions, numTimesVisited] = ht[hashKey]
    else:   # That state is not initialized in the hash table yet
        actions = [0, 0]

        numTimesVisited = 1                 # to prevent divide by 0

    return [actions, numTimesVisited]
...
def main():
    ...
    """
    if len(sys.argv) > 1:          # optional input file for loading trained bird
        filename = sys.argv[1]
        try:
            ...
            print ("Error, need a valid input file")
            sys.exit(2)
    else:                           # Fresh bird to train
        ht = {}                     # New hash table for the states

    gamma = 0.9                     # consider future rewards the most

    for i in range(100):     # Play 100 times at once to train
        pygame.init()
        lastState = None
        ...
        j = 0               # counts number of iterations since last state checked
        while not done:

            # generate states
            if j is 9:     # updating last state every 5th i
                currState = makeKey(bird, pipes[0])
                [Qold, numTimesVisited] = getQold(ht, currState)
                action = getAction(Qold, numTimesVisited)
                if (action is 0):     # If action is up, send KEYUP to make bird climb
                    myEvent = pygame.event.Event(KEYUP)
                    myEvent.key = K_UP
            ...
            if pipe_collision:
                reward = -500       # at least got to pipe
                done = True
            elif 0 >= bird.y or bird.y >= WIN_HEIGHT - Bird.HEIGHT:
                reward = -1000     # didn't even get to pipe, so worst terminal state
                done = True
            elif j is 9:
                reward = 1          # staying alive hey hey hey
            ...
            # increment and update before looping
            if j is 9 or done:
                # update Q if have the previous state
                if lastState:
                    Qsample = reward + gamma * max(Qold[0], Qold[1])
                    Qnew = Qolder
                    Qnew[oldAction] = Qolder[oldAction] + (1/numTimesVisited)* (Qsample - Qolder[oldAction])
                    ht[lastState] = [Qnew, round(numTimesVisited+0.5, 1)]
                lastState = currState
                oldAction = action
                Qolder = Qold
                j = 0
    ...
    # Save bird training to text file before actually exiting
    with open('new_trained.pkl', 'wb') as f:
        pickle.dump(ht, f, pickle.HIGHEST_PROTOCOL)
    f.close()
